<template>
  ...
  <el-dialog :title="titleName" :visible="showDialog" :before-close="btnCancel" style="margin-top: 120px;">
    <el-form
      ...
      :model="form"
    >
      <el-form-item label="省份编号" prop="province_code">
        <el-input v-model="form.province_code" class="input_box" :disabled="true" />
      </el-form-item>
      <el-form-item label="省份所属区域" prop="province_region">
        <el-select v-model="form.province_region" class="input_box">
          <el-option
      ...
      <el-form-item label="省份名" prop="province_name">
        <el-input v-model="form.province_name" class="input_box" />
      </el-form-item>
    </el-form>
    <!-- 弹框底部 -->
...
export default {
  ...
  data() {
    return {
      ...
      rules: {
        province_region: [
          { required: true, message: '请选择省份区域', trigger: 'change' }
        ],
        province_name: [
  ...
  watch: {
    computedProperty() {
      const obj = this.computedProperty
      const arr = []
      obj.map(function(item, index, array) {
        const region = { 'value': item['region_code'],
